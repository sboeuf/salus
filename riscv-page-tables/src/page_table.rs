    ...
    unsafe fn promote_pte(
        ...
        while let Table(t) = entry {
            if t.level().leaf_page_size() != page_size {
                ...
                // Safety: `paddr` references a page uniquely owned by the root `GuestStagePageTable`.
...
impl<T: PagingMode> GuestStagePageTable<T> {
        ...
        F: FnMut(SupervisorPageAddr, PageSize) -> bool,
    {
        ...
        let end = vaddr
            ...
            .unwrap();
        let mut inner = self.inner.lock();
        let mut prev_paddr: Option<SupervisorPageAddr> = None;
        ...
        while va < end {
            ...
            if let Some(addr) = &prev_paddr {
                // Check the current page is contiguous with the previous page
                if addr
                    .checked_add_pages_with_size(1, expected_page_size)
                    .ok_or(Error::AddressOverflow)?
                    != paddr
                {
