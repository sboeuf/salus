...
impl PageTracker {
    ...
    /// Returns true if and only if `addr` is a page owned by `owner` with type `mem_type`.
    pub fn is_blocked_page(
        ...
        let mut page_tracker = self.inner.lock();
        if let Ok(info) = page_tracker.get(addr) {
            info.owner() == Some(owner) && info.mem_type() == mem_type && info.is_blocked()
        } else {
            false
        }
    }

    /// Returns true if and only if `addr` is a page owned by the host with type `mem_type`.
    pub fn is_blocked_shared_page(&self, addr: SupervisorPageAddr, mem_type: MemType) -> bool {
        let mut page_tracker = self.inner.lock();
        if let Ok(info) = page_tracker.get(addr) {
            info.owner() == Some(PageOwnerId::host())
                && info.mem_type() == mem_type
                && info.is_blocked_shared()
        } else {
            false
        }
    }
