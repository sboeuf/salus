...
const ENTRIES_PER_PAGE: u64 = 512;
...
impl<'a, T: GuestStagePagingMode> FinalizedVmPages<'a, T> {
    ...
    pub fn promote_page(&self, page_addr: GuestPageAddr, page_size: PageSize) -> Result<()> {
        ...
        if !(regions.contains(page_addr, end, |r| r == VmRegionType::Confidential)
            || regions.contains(page_addr, end, |r| r == VmRegionType::ConfidentialRemovable)
            || regions.contains(page_addr, end, |r| r == VmRegionType::Shared)
            || regions.contains(page_addr, end, |r| r == VmRegionType::SharedRemovable))
        {
